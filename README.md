...
- IAM permissions to create S3, Lambda, SQS, Glue, and Step Functions.
...
5. Step 5: Set up Data Cleaning (AWS Glue)
6. Locate code in: AWS_Glue_functions_clean-data
7. Navigate to the AWS Glue Console.
8. Create a generic Python Shell job.
9. Paste the code from the repository folder.
10. Permissions: Ensure the Glue Role has s3:GetObject (on raw) and s3:PutObject (on cleaned).
11. Configuration:
...
**Monitoring:**
Check CloudWatch Logs for Lambda execution details. Check the SQS Console to ensure messages are being consumed (Flight messages should go to 0). Check S3 to see files populating in the raw and cleaned_data folders.
